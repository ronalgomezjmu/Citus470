...
TERMINFO="/scratch/postgres-citus/local/share/terminfo"
...
(*.[0-9]*)
	...
	if test ! -f "/scratch/postgres-citus/build/ncurses-6.4/man_alias.sed" ; then
cat >>/scratch/postgres-citus/build/ncurses-6.4/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /scratch/postgres-citus/build/ncurses-6.4/man_alias.sed"
	fi
	...
	aliases=`sed -f "$top_srcdir/man/manlinks.sed" "$inalias" |sed -f "/scratch/postgres-citus/build/ncurses-6.4/man_alias.sed" | sort -u; test "$inalias" = "$nCurses" && echo curses`
	...
	sed	-f "/scratch/postgres-citus/build/ncurses-6.4/man_alias.sed" \
		< "$i" >$TMP
